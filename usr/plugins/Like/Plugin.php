...
class Like_Plugin implements Typecho_Plugin_Interface
{
    ...
    public static function theLike($link = true){
        ...
        if( $link ){
            ...
            echo '<div class="'.$settings->likeClass.'">'.
                '<a href="javascript:;" data-pid="'.$cid.'" class="action">'.
        ...
        if ($posts) {
            foreach ($posts as $post) {
                ...
                if($showlink == true){
                    ...
                    $span = empty($before) && empty($after) ? '' : "<span style='font-size:70%'>$before $post_likes $after</span>";
              		echo "<li><a href='$permalink' title='$post_title'>$post_title</a>$span</li>\n";
              	}
            }
