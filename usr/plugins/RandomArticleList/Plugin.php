...
class RandomArticleList_Plugin implements Typecho_Plugin_Interface
{
	 ...
	 * @return void
	 */
	public static function parse($format='<li category="{category}"><a href="{permalink}">{title}</a></li>')
	{
		...
		if ($xml) {
			...
			if(time()-$time_dl<$option->cache){ //如果时间间隔小于设定值，则直接从xml文件读取
				...
				{		   
					echo str_replace(array('{permalink}','{title}','{category}'),array($rd->link[0],$rd->title[0],$rd->category[0]),$format);
				}
			}else{
				self::qy_datebase($format,$option,$file);
			}
		} else {
			self::qy_datebase($format,$option,$file);
		}
	...
	private static function qy_datebase($format,$option,$file){  
		/**获取文章数目设定*/		   
		if ($option->numset == 'a'){
		...
		foreach($rand as $index){		
			...
			$title = $option->title_length ? self::cutstr($value['title'],$option->title_length) : $value['title'];
			echo str_replace(array('{permalink}','{title}','{category}'),array($value['permalink'],$title,$value['categories'][0]['name']),$format);
			$rd=$xml->addChild('rd');
			$rd->addChild('title',$title);
			$rd->addChild('link',$value['permalink']);		
			$rd->addChild('category',$value['categories'][0]['name']);
		}
